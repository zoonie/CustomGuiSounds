...
public abstract class GuiScrollableList
{
	...
	public GuiScrollableList(Minecraft client, int width, int height, int top, int bottom, int left, int entryHeight)
	{
		this.client = client;
	...
	public void drawScreen(int mouseX, int mouseY, float p_22243_3_)
	{
		...
		{
			if(this.initialMouseClickY == -1.0F && mouseX > boxLeft && mouseX < (boxLeft + this.listWidth))
			{
				boolean var7 = true;
		...
		{
			while(mouseX > boxLeft && mouseX < boxRight && Mouse.next())
			{
